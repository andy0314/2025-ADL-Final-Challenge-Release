...
def parse_args():
    ...
    parser.add_argument("--output", type=str, default="./data2/reward_dataset.jsonl",
        ...
        for response_a, response_b in combinations(response_list, 2):
            ...
            if reward_a != reward_b:
                if reward_a > reward_b:
                    chosen = text_a
                    rejected = text_b
                    chosen_score = reward_a # Capture score A
                    rejected_score = reward_b # Capture score B
                else:
                    chosen = text_b
                    rejected = text_a
                    chosen_score = reward_b # Capture score B
                    rejected_score = reward_a # Capture score A
                    
                preference_pairs.append({
                    "prompt": prompt,
                    "chosen": chosen,
                    "rejected": rejected,
                    "chosen_score": chosen_score, # NEW: 加入 chosen 的分數
                    "rejected_score": rejected_score # NEW: 加入 rejected 的分數
                })
...
def main():
    ...
    print("輸出格式: {prompt, chosen, rejected, chosen_score, rejected_score}")
